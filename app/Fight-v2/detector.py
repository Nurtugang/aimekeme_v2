...
Модель загружается один раз при старте; predict декодирует settings.expected_frames
base64-кадров, препроцессит и классифицирует клип. GPU-инференс под локом (один процесс,
одна видеокарта).

ВАЖНО про кадры: они должны быть РАВНОМЕРНО распределены по окну ~settings.fight_window_seconds
секунд потока, а не последними N кадрами подряд — иначе X3D-M даёт плоский, неинформативный
score независимо от содержимого (см. полный контракт и диагностику в app/config.py у
fight_window_seconds, и в app/fight/schemas.py у DetectionRequest.frames). Этот модуль
принимает кадры как есть и НЕ проверяет их временной охват — это ответственность вызывающей
стороны (клиента, формирующего запрос к /detect/fight).
"""
